...
(function ($) {
    ...
    $.carrousel.prototype = {
        ...
        _init: function (options) {
            ...
            this.$wrapper1 = this.$el.find('.dg-wrapper a:gt(1)');
            this.$items = this.$wrapper.children();
            this.$items1 = this.$wrapper1
            ...
            $('#carousel-bigimg-generic').hammer().on('click', function(){
		        _self1 = this;
		        console.log(this);
		    });
			$('#carousel-bigimg-generic').click();
            //手势滑动
            $('#dg-container').hammer().on('swipeleft', function(){
        		_self._navigate("next")
			   $('#carousel-bigimg-generic .item').eq( _self.indexB ).addClass("active").siblings().removeClass("active");
			   $('#carousel-bigimg-generic li').eq( _self.indexB ).addClass("active").siblings().removeClass("active");
		    });
		    $('#dg-container').hammer().on('swiperight', function(){
        		_self._navigate("prev")
//      		console.log( $(this).next() )
//      		var obj = "carousel-bigimg-generic";
//      		var guide = "prev";
//      		console.log(_self.indexB)
//      		$('#'+obj).click()
//      		swiperleft(obj,guide,_self1);
////			    $('#'+obj).click();
				$('#carousel-bigimg-generic .item').eq( _self.indexB ).addClass("active").siblings().removeClass("active");;
			   	$('#carousel-bigimg-generic li').eq( _self.indexB ).addClass("active").siblings().removeClass("active");
		    });
		    //链接hammer.min.js + js/jquery.hammer.js时用下面两个方法实现手势滑动
		    function swiperleft( obj,guide,_self1 ){
		    	if( obj == "dg-container"){
			    	$('#'+obj).hammer().on('click', function(){
				        _self._navigate( guide );
//		    			console.log(obj)
				    });
		    	}else if( obj == "carousel-bigimg-generic" ){
		    		$(_self1).carousel( guide );
//		    		console.log(_self.indexB)
//		    		console.log("prve")
		    	}
		    }
			...
			//点击放大
		    $('#dg-container .dg-wrapper').on("click","a",function(){
//		    	console.log( $(this).attr("class") )
		    	var index = $(this).index("#dg-container .dg-wrapper a");
		    	let fristPic = $("#dg-container .dg-wrapper a").length-2
		    	if( _self.itemsCount == 2 ){
		    		if( $(this).css("transform") == "matrix(1, 0, 0, 1, 0, 0)" ){
		    			$(".big-swiper-black").addClass("big-swiper-show")
		    			console.log($(".big-swiper-black").attr("class"))
		    			$('#carousel-bigimg-generic .item').eq( 1 - _self.indexB ).addClass("active").siblings().removeClass("active");;
			   			$('#carousel-bigimg-generic li').eq( 1 - _self.indexB ).addClass("active").siblings().removeClass("active");
		    		}
		    	}else if($(this).css("transform") == "matrix(1, 0, 0, 1, 0, 0)" && fristPic == index ){
		    		$(".big-swiper-black").addClass("big-swiper-show")
		    		$('#carousel-bigimg-generic .item').eq( _self.indexB ).addClass("active").siblings().removeClass("active");;
			   		$('#carousel-bigimg-generic li').eq( _self.indexB ).addClass("active").siblings().removeClass("active");
		    	}else if ( !$(this).hasClass("dg-transition")){
		    		$(".big-swiper-black").addClass("big-swiper-show")
		    		$('#carousel-bigimg-generic .item').eq( _self.indexB ).addClass("active").siblings().removeClass("active");;
			   		$('#carousel-bigimg-generic li').eq( _self.indexB ).addClass("active").siblings().removeClass("active");
		    	}
...
//			   			$('#carousel-bigimg-generic li').eq( 1 - _self.indexB ).addClass("active").siblings().removeClass("active");
//		    		}
//		    	}else if( $('#dg-container .dg-wrapper a').eq( index).hasClass("dg-center") ){
//		    		$(".big-swiper-black").addClass("big-swiper-show")
//		    		$('#carousel-bigimg-generic .item').eq( _self.indexB ).addClass("active").siblings().removeClass("active");;
//			   		$('#carousel-bigimg-generic li').eq( _self.indexB ).addClass("active").siblings().removeClass("active");
//		    	}
		    ...
		    //点击放大消失
		    $(".big-swiper-black").click(function(){
		    	$(this).removeClass("big-swiper-show")
		    })
        ...
        _loadEvents: function () {
            ...
            this.$wrapper.on('webkitTransitionEnd.carrousel transitionend.carrousel OTransitionEnd.carrousel', function () {
                _self.$currentItm.addClass('dg-center');
           		$("#dg-container .dg-wrapper a").removeClass("dg-index")
           		
           		
           		_self.$currentItm.next().addClass('dg-index');
           		_self.$currentItm.prev().addClass('dg-index');
           		
           		if( _self.$currentItm.next().length != 1 ){
   					_self.$items.eq( _self.itemsCount-1 ).addClass("dg-index")
   					console.log(_self.$currentItm.next().length)
	           	}
           		if( _self.$currentItm.prev().length != 1 ){
           			_self.$items.eq( 0 ).addClass("dg-index")
           			console.log(_self.$currentItm.prev().length)
           		}
